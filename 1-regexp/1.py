...
def find_references(filepath):
    ...
    with open(filepath, 'r') as bill_file:
        ...
        # be careful with a sentence:  W ustawie z dnia 20 czerwca 1985 r. - Prawo o ustroju sądów powszechnych
        # brackets = regex.findall(r"(?P<sentence>[^\.\)]*)(?P<dz>\(Dz.[\t\p{Zs}]*U.[\t\p{Zs}]*[^\)]*\))[^\.]*", bill, flags=regex.DOTALL)

        # extracting year
        # brackets = regex.findall(
        #     r"(?:(?P<year>\d{4})[\t\p{Zs}\xA0\n]*(?>roku|r\.)*)*.*"
        #     r"[\t\p{Zs}]*(?>roku|r\.)"
        #     r"(?P<sentence>[^\.]*(?=\d{4}[\t\p{Zs}]*)*[^\.\(]*)"  # find part of the sentence with year
        #     r"(?P<dz>Dz\.[\t\p{Zs}\xA0\n]*U\.[\t\p{Zs}\xA0\n]*"  # match 'Dz. U. '
        #     r"\X*?(?:\d\.|[^\t\p{Zs}]*[^r](?=\.[\t\p{Zs}]*\p{Lu})|\)|$))",  # match until 'D. | . <Uppercase> | ) | $ '
        #     # bill)
        #     'Zmiany wymienionej ustawy zostały ogłoszone w Dz.U. z 2016 r.poz. 2225  oraz z\n'
        #     '2017 r. poz. 88, 244, 379, 708, 768, 1086 i 1321.\n'
        #     '44 ustawy z dnia 13 lipca \n1990 roku o prywatyzacji przedsiębiorstw państwowych (Dz.U. Nr 51, poz.'
        #     '\n298 oraz z 1991 r. Nr 60, poz. 253 i Nr 111, poz. 480) z wyłączeniem'
        #     '\nprzepisów art. 19 ust. 1 zdanie drugie i art. 28. 4 ustawy z dnia 13 lipca '
        #     '\n1990 r. o prywatyzacji przedsiębiorstw państwowych (Dz.U. Nr 51, poz.'
        #     '\n298 oraz z 1991 r. Nr 60, poz. 253 i Nr 111, poz. 480) z wyłączeniem'
        #     '\nprzepisów art. 19 ust. 1 zdanie drugie i art. 28.')


        # extracting year copy
        # brackets = regex.findall(
        #     r"(?:(?P<year>\d{4})[\t\p{Zs}\xA0\n]*(?>roku|r\.)*)*.*"
        #     r"[\t\p{Zs}]*(?>roku|r\.)"
        #     r"(?P<sentence>[^\.]*(?=\d{4}[\t\p{Zs}]*)*[^\.\(]*)"  # find part of the sentence with year
        ...
        #     r"\X*?(?:\d\.|[^\t\p{Zs}]*[^r](?=\.[\t\p{Zs}]*\p{Lu})|\)|$))",  # match until 'D. | . <Uppercase> | ) | $ '
        #     bill)
        #     'Zmiany wymienionej ustawy zostały ogłoszone (Dz.U. z 2016 r. poz. 2225  oraz z\n'
        #     '2017 r. poz. 88, 244, 379, 708, 768, 1086 i 1321).\n'
        #     '44 ustawy z dnia 13 lipca \n1990 roku o prywatyzacji przedsiębiorstw państwowych (Dz.U. Nr 51, poz.'
        #     '\n298 oraz z 1991 r. Nr 60, poz. 253 i Nr 111, poz. 480) z wyłączeniem'
        #     '\nprzepisów art. 19 ust. 1 zdanie drugie i art. 28. 4 ustawy z dnia 13 lipca '
        ...
        # # # extracting year, sentence and reference
        # brackets = regex.findall(
            # r"(?=(?P<year>\d{4})[\t\p{Zs}\xA0\n]*(?>roku|r\.)).*" # match XXXX r. or XXXX roku
            # r"[\t\p{Zs}]*(?>roku|r\.)"
            # r"(?P<sentence>\X*(?=\d{4}[\t\p{Zs}]*)*[^\.\(]*)"  # find part of the sentence with year
            # r"(?P<dz>Dz\.[\t\p{Zs}\xA0\n]*U\.[\t\p{Zs}\xA0\n]*"  # match 'Dz. U. '
            # r"\X*?(?:\d\.|[^\t\p{Zs}]*[^r](?=\.[\t\p{Zs}]*\p{Lu})|\)|$))",  # match until 'D. | . <Uppercase> | ) | $ '
            # bill)
            # # 'Zmiany wymienionej ustawy zostały ogłoszone w .Dz.U. z 2016 r. poz. 2225  oraz z \n'
        ...
        #     r"(?P<rest>\X*?"
        #     r"\))",
        #     'Zmiany wymienionej ustawy zostały 2000 roku ogłoszone w (Dz.U. z 2016 r. poz. 2225 oraz z \n'
        #     '2017 r. poz. 88, 244, 379, 708, 768, 1086 i 1321).\n'
        #     'Zdanie po nic. '
...
# DONT CHANGE!
        ...
        #     # '\n298 oraz z 1991 r. Nr 60, poz. 253 i Nr 111, poz. 480) z wyłączeniem'
        #     # '\nprzepisów art. 19 ust. 1 zdanie drugie i art. 28.'
        #     'Zdanie to 1)Dz. U. Nr 20. Tak to jest, że poz. 20 i nic. Zdanie 3 (bo tak) wypada.'
        # )
